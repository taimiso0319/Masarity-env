...
# Path to the custom Masarity $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.masarity.$(hostname).cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.masarity.$(hostname).crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.masarity.$(hostname).key"
...
BASE_CNF="
    [ ca ]
    default_ca = ca_masarity_$(hostname)

    [ ca_masarity_$(hostname) ]
    dir           = $PATH_SSL
    certs         = $PATH_SSL
...
then
    ...
    # Finally, generate the private key and certificate signed with the Masarity $(hostname) Root CA.
