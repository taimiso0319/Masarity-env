class Masarity
    def self.configure(config, settings)
        ...
        config.vm.provider 'virtualbox' do |vb|
            vb.name = settings['name'] ||= 'masarity-7'
        ...
        #         v.vmx['displayName'] = settings['name'] ||= 'masarity-7'
        #         v.vmx['memsize'] = settings['memory'] ||= 1024
        #         v.vmx['numvcpus'] = settings['cpus'] ||= 1
        #         v.vmx['guestOS'] = 'centos7-64'
        #         if settings.has_key?('gui') && settings['gui']
        #             v.gui = true
        #         end
        #     end
        ...
        #     h.vmname = settings['name'] ||= 'masarity-7'
        ...
        #     v.name = settings['name'] ||= 'masarity-7'
        ...
        if settings.include? 'keys'
            if settings['keys'].to_s.length.zero?
                puts 'Check your Masarity.yaml file, you have no private key(s) specified.'
                exit
            end
            settings['keys'].each do |key|
                if File.exist? File.expand_path(key)
                    ...
                    puts 'Check your Masarity.yaml (or Homestead.json) file, the path to your private key does not exist.'
                    exit
                end
            end
        end
        ...
        if settings.include? 'folders'
            settings['folders'].each do |folder|
                if File.exist? File.expand_path(folder['map'])
                    ...
                    config.vm.provision 'shell' do |s|
                        s.inline = ">&2 echo \"Unable to mount one of your folders. Please check your folders in Masarity.yaml\""
                    end
                end
            end
        end
        ...
        if settings.has_key?('backup') && settings['backup'] && (Vagrant::VERSION >= '2.1.0' || Vagrant.has_plugin?('vagrant-triggers'))
            ...
            settings['databases'].each do |database|
                Masarity.backup_mysql(database, "#{dir_prefix}/mysql_backup", config)
                Masarity.backup_postgres(database, "#{dir_prefix}/postgres_backup", config)
            end
        end
    ...
    def self.backup_postgres(database, dir, config)
        ...
        config.trigger.before :destroy do |trigger|
            ...
            trigger.run_remote = { inline: "mkdir -p #{dir} && echo localhost:5432:#{database}:masarity:secret > ~/.pgpass && chmod 600 ~/.pgpass && pg_dump -U masarity -h localhost #{database} > #{dir}/#{database}-#{now}.sql" }
        end
    end
end